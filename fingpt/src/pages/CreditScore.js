...
function CreditScore() {
  ...
  const calculateCreditScore = (paymentHistory, creditUtilization) => {
    ...
    return Math.round(
      Math.min(maxScore, baseScore + paymentHistoryImpact + utilizationImpact)
    );
  };

  const projectCreditScore = (
    baseScore,
    paymentHistory,
    creditUtilization,
    months
    ...
    const habitFactor =
      (paymentHistory / 100) * 0.7 + ((100 - creditUtilization) / 100) * 0.3;
    const improvement =
      maxImprovement * habitFactor * (1 - Math.exp(-improvementRate * months));
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            <Paper elevation={3} sx={{ p: 3 }}>
              ...
              <Box mt={2}>
                ...
                <Typography
                  ...
                  align="center"
                  style={{
                    color: getStatusColor(getCreditScoreStatus(currentScore)),
                  }}
                >
          ...
          <Grid item xs={12}>
            <Paper elevation={3} sx={{ p: 3 }}>
              ...
              <Box sx={{ width: "100%", height: 300 }}>
                <LineChart
                  ...
                  yAxis={[
                    {
                      min:
                        Math.min(...creditScoreData.map((d) => d.score)) - 20,
                      max:
                        Math.max(...creditScoreData.map((d) => d.score)) + 20,
                    },
                  ]}
                  series={[
                    {
              ...
              <Typography variant="body2" mt={2}>
                This projection is based on your current payment history (
                {paymentHistory}%) and credit utilization ({creditUtilization}
                %).
                {paymentHistory < 90 &&
                  " Your payment history is below ideal levels. Improving this can significantly boost your score."}
                {creditUtilization > 30 &&
                  " Your credit utilization is higher than recommended. Reducing this can help improve your score."}
                {paymentHistory >= 90 &&
                  creditUtilization <= 30 &&
                  " You're on the right track! Maintaining these good habits should help your score improve over time."}
              </Typography>
              <Typography variant="body2" mt={2}>
                Remember, this is an estimate. Actual results may vary based on
                many factors. For the best improvement:
                <ul>
              ...
              <Grid container spacing={2} justifyContent="center">
                ...
                <Grid item xs={12} sm={6} md={4}>
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        "https://www.experian.in/consumer/credit-report",
                        "_blank"
                      )
                    }
                  >
        ...
        <Box mt={4}>
          <Typography variant="body2" color="textSecondary" align="center">
            ...
            <br></br>
            For accurate credit scores and personalized advice, consult with
