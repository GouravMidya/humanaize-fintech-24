...
import {
  Box, Typography, TextField, Button, Grid, Paper, Slider, List, ListItem, ListItemText,
  Divider, Alert, IconButton, MenuItem, Select, CircularProgress, Container
} from '@mui/material';
...
const BudgetOptimizer = () => {
  ...
  const fetchAndSetExpenses = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_NODEURL}/getexpensesdaterange`, {
        userId,
      ...
      const totalExpense = fetchedExpenses.reduce((sum, expense) => sum + parseFloat(expense.amount), 0);
      ...
      const totalPercentage = Object.values(expensePercentages).reduce((sum, value) => sum + value, 0);
  ...
  const BudgetSummary = ({ title, expenses, totalIncome, isOptimized = false, originalExpenses }) => {
                    ...
                    secondary={
                      <span>
                        ...
                        {isOptimized && originalExpenses && (
                          <span style={{ 
                            color: percentage > originalExpenses[category] ? 'green' : 
                                   percentage < originalExpenses[category] ? 'red' : 'inherit', 
                            marginLeft: '5px' 
                          }}>
            ...
            <>
              <Grid item xs={12} md={6}>
                <Paper elevation={3} sx={{ p: 3 }}>
                  ...
                  <Button startIcon={<AddIcon />} onClick={addIncome} variant="outlined" fullWidth>
                    Add Income
                  </Button>
                  ...
                  {Object.entries(expenses).map(([category, percentage]) => (
                    category !== 'Savings' && (
                      <Box key={category} sx={{ mb: 2 }}>
                        <Typography gutterBottom>{category} ({percentage.toFixed(2)}%)</Typography>
                        <Slider
                          value={percentage}
                  ...
                  <Button startIcon={<AddIcon />} onClick={addExpense} variant="outlined" fullWidth>
